...
app.controller('myCtrl', ['$scope', '$rootScope', 'BusinessService', function ($scope, $rootScope, BusinessService) {
    ...
    //列表（1）属性
    $scope.param={};
    $scope.param.groupId=$scope.query.groupId;
    $scope.orderBy="joinDate,1";
    $scope.mainKey="userId"; //方法需要用到,表示根据哪个字段来操作
    $scope.listUrl="/userMgmt/listUserGroupMember.do";
    $scope.changeGridData=function() {

    };
    //列表（2）属性
    $scope.param2={};
    $scope.param2.groupId=$scope.query.groupId;
    $scope.orderBy2="userId,-1";
    $scope.mainKey2="userId"; //方法需要用到,表示根据哪个字段来操作
    $scope.listUrl2="/userMgmt/listUserForUserGroup.do";
